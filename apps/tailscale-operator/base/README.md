...
Each Ingress in this repo creates a separate tailnet node, which lets you share
individual admin services from the Tailscale admin console.

## Sharing ACLs

Each tailnet ingress is tagged with `tag:k8s` and a service-specific sharing tag:
...
1. Share the specific machine from the Tailscale admin console.
...
Sharing makes the selected machine visible to the recipient user in their
Tailscale client. The grant limits what that user can connect to. Shared machines
are visible only to the invited user, not to every member of the recipient's
tailnet.
